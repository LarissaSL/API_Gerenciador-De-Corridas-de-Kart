...
public interface CheckRepository extends JpaRepository<Check, Long> {
    ...
    @Query("SELECT COUNT(c) > 0 FROM Check c WHERE c.inscricao.corrida.id = :idCorrida")
    boolean existsCheckInsByCorridaId(Long idCorrida);
    ...
    @Query("SELECT c FROM Check c WHERE c.inscricao.corrida.id = :idCorrida")
    List<Check> listarTodosOsCheckInsPorIdCorrida(Long idCorrida);
